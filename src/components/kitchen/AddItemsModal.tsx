...
type AddItemsModalProps = {
    ...
    onClose: () => void;
    menuItems: MenuItem[];
    onAdd: (items: Record<number, number>) => void; // menu_item_id -> qty
};

export default function AddItemsModal({ isOpen, onClose, menuItems, onAdd }: AddItemsModalProps) {
    ...
    const confirm = () => {
        if (Object.keys(cart).length === 0) { onClose(); return; }
        onAdd(cart);
        setCart({});
        onClose();
    };
                ...
                <div className="border-t p-3 flex-none">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col items-end gap-2 w-48">
                            ...
                            <button className="px-4 py-2 bg-green-600 text-white rounded w-full" disabled={Object.keys(cart).length === 0} onClick={confirm}>
                                Add to Order
                            </button>
                        </div>
                    </div>
